...
class TickerSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_financial_data(self, obj):
        return self.context['request'].build_absolute_uri(reverse('getstocksapp:ticker-financial-info', args=[obj.pk]))
...
class MarketSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_tickers(self, obj):
        ...
        return {
            ticker.ticker_name: self.context['request'].build_absolute_uri(reverse('getstocksapp:ticker-detail', args=[ticker.pk])) 
...
class UserSerializer(serializers.HyperlinkedModelSerializer):
    ...
    url = serializers.HyperlinkedIdentityField(view_name='getstocksapp:user-detail')
    ...
    def get_wallets(self, obj):
        ...
        return {wallet.name: self.context['request'].build_absolute_uri(reverse('getstocksapp:wallet-detail', args=[wallet.pk])) for wallet in wallets_queryset}
...
class WalletSerializer(serializers.HyperlinkedModelSerializer):
    ...
    url = serializers.HyperlinkedIdentityField(view_name='getstocksapp:wallet-detail')
    ...
    def get_records(self, obj):
        ...
        return {record.name: self.context['request'].build_absolute_uri(reverse('getstocksapp:wallet-record-detail', args=[record.pk])) for record in records_queryset}
    
    def get_owner(self,obj):
        owner_user = obj.owner
        return f"{owner_user.username} : {self.context['request'].build_absolute_uri(reverse('getstocksapp:user-detail', args=[owner_user.pk]))}"

    def get_guests(self, obj):
        guests_queryset = obj.guests.all()
        return {guest.username: self.context['request'].build_absolute_uri(reverse('getstocksapp:user-detail', args=[guest.pk])) for guest in guests_queryset}
 

class WalletRecordSerializer(serializers.HyperlinkedModelSerializer):
    wallet = serializers.SerializerMethodField()
    ticker = serializers.SerializerMethodField()
    url = serializers.HyperlinkedIdentityField(view_name='getstocksapp:wallet-record-detail')
    ...
    def get_wallet(self, obj):
        return f"{obj.wallet.name} : {self.context['request'].build_absolute_uri(reverse('getstocksapp:wallet-detail', args=[obj.wallet.pk]))}"
    
    def get_ticker(self, obj):
        return f"{obj.ticker.ticker_name} : {self.context['request'].build_absolute_uri(reverse('getstocksapp:wallet-detail', args=[obj.ticker.pk]))}"
